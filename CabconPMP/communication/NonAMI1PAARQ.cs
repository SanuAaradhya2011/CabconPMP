...
public class NonAMI1PAARQ : basecosem
{
    public override List<byte> GetPacket(ConfigureData configureData)
    {
        ...
        List<byte> SECURITYTAG = new List<byte>()
                { 0x8A, 0X02, 0X07, 0X80, 0X8B, 0X07, 0X60, 0X85, 0X74, 0X05, 0X08, 0X02, 0X01, 0XAC, 0X0A, 0X80 };

        List<byte> SEEDDATA = new List<byte>()
                { 
                    //SEED Length, SEED Data
                    0x08, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 
                };
