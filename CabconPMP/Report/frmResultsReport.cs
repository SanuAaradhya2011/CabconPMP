...
namespace CabconPMP
{
    public partial class frmResultsReport : Form
    {
        ...
        private void frmResultsReport_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (cmbSortBy.Items.Count >=2) cmbSortBy.SelectedIndex = 2;
                else cmbSortBy.SelectedIndex = 0;
        ...
        private void lblFind_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (cmbStatus.SelectedIndex <= 0) objettER.FinalResult = "";//-------Show All
                    ...
                    if (cmbSortBy.SelectedIndex == 0) { objettER.ExecutionTestID = txtFind.Text.Trim();  ds = objER.SelectFromtabExecutionResult_TestID(objettER); }
                    else if (cmbSortBy.SelectedIndex == 1) { objettER.CustomerName = txtFind.Text.Trim();  ds = objER.SelectFromtabExecutionResult_CustomerName(objettER); }
                    else if (cmbSortBy.SelectedIndex == 2)
                    {


                        if (txtFind.Text.Length >= 6 && txtfindto.Text.Length >= 6)
                        {
                            string[] dtStart = txtFind.Text.Trim().Split('/');
                            ...
                            if (isvalidStartDateTime && isvalidEndDateTime)
                            {
                                ...
                                ds = objER.SelectFromtabExecutionResult_ExecutionDate(objettER);
                            }
                        }
                        else { isvalidStartDateTime = false; isvalidEndDateTime = false; }
                        if (!isvalidStartDateTime || !isvalidEndDateTime)
                        {
                            MessageBox.Show("Invalid Date Time Format, Please Provide Input in DD/MM/YYYY Format Only !", "Cabcon PMP", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                            return;
                        }
                        
                    }
                    else if (cmbSortBy.SelectedIndex == 3) { objettER.PCBAID = txtFind.Text.Trim();  ds = objER.Select_GetExecutionResult_LIKE_PCBAID(objettER); }
                    else if (cmbSortBy.SelectedIndex == 4) { objettER.MeterID = txtFind.Text.Trim(); ds = objER.Select_GetExecutionResult_LIKE_MeterID(objettER); }
                    else if (cmbSortBy.SelectedIndex == 5) { objettER.LogedUserID = txtFind.Text.Trim(); ds = objER.Select_GetExecutionResult_LIKE_LogedUserID(objettER); }
        ...
        {
            if (cmbSortBy.SelectedIndex == 2) { txtFind.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now.AddDays(-(DateTime.Now.Day - 1))); txtfindto.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now); txtfindto.Enabled = true; }
            else { txtFind.Text = ""; txtfindto.Text = ""; txtfindto.Enabled = false; }
        }
