...
namespace CabconPMP
{
    public partial class frmProductionStausReport : Form
    {
        ...
        private void lblFind_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Please Select A Valid Search Condition !", "Cabcon PMP", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                cmbSortBy.Focus();
                return;
            }
            ...
            try
            {
                ...
                if (cmbSortBy.SelectedIndex == 1) 
                {
                    ...
                    else if (cmbSortBy.SelectedIndex == 0)
                    {
                        ...
                        if (!isvalidStartDateTime || !isvalidEndDateTime)
                        {
                            MessageBox.Show("Invalid Date Format, Please Provide Valid 10 Digit Date in DD/MM/YYYY Format Only  !", "Cabcon PMP", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                            return;
                        }
                    ...
                    {
                        MessageBox.Show("Data Not Found OR Unable to Retrive Data !", "Cabcon PMP", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
