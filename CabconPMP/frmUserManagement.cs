...
namespace CabconPMP
{
    public partial class frmUserManagement : Form
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("Do You Want To Save/ Update the User Details ? ", "Cabcon PMP", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No) return;
            if (objum.Insert_UserDetails(objetyusermgt) != "")
            {
                MessageBox.Show("Unable To Save/ Update Records !", "Cabcon PMP", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            FillUserDetails();
            MessageBox.Show("User Details Saved !", "Cabcon PMP", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        ...
        private void DGVUsermgt_DoubleClick(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("Do You Want To View/ Edit the Selected User Details ? ", "Cabcon PMP", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No) return;
        ...
        private bool IsValidUserDetails()
        {
            ...
            {
                MessageBox.Show("Please Select Valid User Type !", "Cabcon PMP", MessageBoxButtons.OK, MessageBoxIcon.Error);
                cmbUserType.Focus();
                return false;
            }
            if (txtUserID.Text.Trim().Length < 3)
            {
                MessageBox.Show("Please Enter Valid User ID, Length Should be >= 3  !", "Cabcon PMP", MessageBoxButtons.OK, MessageBoxIcon.Error);
                txtUserID.Focus();
                return false;
            }
            if (txtnewPassword.Text.Trim().Length < 6)
            {
                MessageBox.Show("Please Enter Valid Password, Length Should be >= 6  !", "Cabcon PMP", MessageBoxButtons.OK, MessageBoxIcon.Error);
                txtnewPassword.Focus();
                return false;
            }
            if (txtnewPassword.Text.Trim() != txtConfirmnewPassword.Text.Trim())
            {
                MessageBox.Show("New Password and Confirm Password Not Matched, Try Again !", "Cabcon PMP", MessageBoxButtons.OK, MessageBoxIcon.Error);
                txtnewPassword.Focus();
                return false;
            }
            return true;
