            ...
            try
            {
                ...
                if (objexeresult.ExecutionProcedureType.IndexOf(StaticVariables.TestType_OT) < 0)
                {
                    if (!isComError && !ExecutionWithOutTravellerStage)
                    {
                        ...
                        {
                            //--------------------For OLD Sapphire-------------------------------------
                            if (!objccmdmethod.SetTravelerData_3PhaseDLMS(selectedControlIDX, TravelerStage, DGVParaLists)) { MessageBox.Show("Unable To Set Production Stage Flag, Meter May at Invalid Production Stage !", "L+G PMP", MessageBoxButtons.OK, MessageBoxIcon.Error); return false; }
                        }
                        else
                        {
                            if (!objccmdmethod.SetTravelerData_3Phase(selectedControlIDX, TravelerStage, DGVParaLists)) { MessageBox.Show("Unable To Set Production Stage Flag, Meter May at Invalid Production Stage !", "L+G PMP", MessageBoxButtons.OK, MessageBoxIcon.Error); return false; }
                        }
                    }
