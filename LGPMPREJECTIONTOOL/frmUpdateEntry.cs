...
namespace LGPMPREJECTIONTOOL
{
    ...
    public partial class frmUpdateEntry : Form
    {
        ...
        private void btnInsert_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (txtError_Name.ReadOnly == false && !String.IsNullOrEmpty(txtError_Name.Text.Trim()))
                {
                    ...
                    try
                    {
                        ...
                        if (frmobj.Password.Equals(password))
                        {
                            ...
                            if (!bSuccess) { MessageBox.Show("Unable to Upload Data!", "L+G PMP", MessageBoxButtons.OK, MessageBoxIcon.Information); return; }
                        }
                        else
                        {
                ...
                if (!(MessageBox.Show(string.Format("Do you want to upload this data?\n\nPCBAID - {0}\n\nProduction Stage - {1}\n\nMeter Type - {2}\n\nError Type - {3}\n\nError State - {4}\n\nCustomer Name - {5}"
                                                                ...
                                                                , string.IsNullOrEmpty(txtCustomer.Text.Trim()) ? "None" : txtCustomer.Text), "L+G PMP Rejection", MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)) { return; }

                bSuccess = balerrorobj.InsertToRejectionTable(errentity);
                if (bSuccess)
                { 
                    MessageBox.Show("Data Uploaded Successfully!","L+G PMP",MessageBoxButtons.OK,MessageBoxIcon.Information); 
                }
                else { MessageBox.Show("Unable to Upload Data!", "L+G PMP", MessageBoxButtons.OK, MessageBoxIcon.Exclamation); }
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "L+G PMP", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private bool ValidateItems()
        {
            ...
            if (!bvalid) { MessageBox.Show("Please Enter All required Fields","L+G PMP", MessageBoxButtons.OK, MessageBoxIcon.Information); }
