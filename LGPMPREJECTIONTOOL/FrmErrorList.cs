    ...
    public partial class frmErrorList : Form
    {
        ...
        private void DisplayRejectionList(Dictionary<string, string> param)
        {
            try
            {
                ...
                if (tableRejectionMaster == null) { MessageBox.Show("Unable to Load Data!", "L+G PMP", MessageBoxButtons.OK, MessageBoxIcon.Error); return; }
        ...
        private void viewErrorTypesTableToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                lblQueryDateTime.Text = DateTime.Now.ToString(DateTimeFormats.DATETIME_FORMAT_DISPLAY, CultureInfo.InvariantCulture);
                if (errortable == null) { MessageBox.Show("Unable to Load Data!", "L+G PMP", MessageBoxButtons.OK, MessageBoxIcon.Information); return; }
        ...
        private void saveToolStripMenuItemExcel_Click(object sender, EventArgs e)
        {
            try
            {
                if (dgvRejectionList.DataSource == null) { MessageBox.Show("No data to Save!", "L+G PMP",MessageBoxButtons.OK, MessageBoxIcon.Information); return; }
                bool bLandscape = false;
        ...
        {
            if (MessageBox.Show("File Saved Successfully! Open File?", "L+G PMP", MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)
        ...
        private void deleteRejectionRecordToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    if (!(MessageBox.Show("Are you sure you want to Delete this record? \n This Record and all associated history will be deleted!", "L+G PMP", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) == DialogResult.Yes)) { return; }
                    ...
                    {
                        MessageBox.Show("Record Deleted Successfully!", "L+G PMP", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        displayRejectionListToolStripMenuItem_Click(new object(), new EventArgs());
                    }
                    else
                        MessageBox.Show("Record could not be deleted!", "L+G PMP", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                else
                    MessageBox.Show("Wrong password!", "L+G PMP", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "L+G PMP", MessageBoxButtons.OK, MessageBoxIcon.Error);
