...
namespace PMPSM310
{
    public partial class frmActionSM310 : Form
    {
        int test_index = 0;
       ...
       // GlobalMethods StaticVariablesvar = new GlobalMethods();
        ...
        private void btnStart_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (finalExecutionStatus) pbExecutionStatus.Image = PMPSM310.Properties.Resources.ExecutionPass;
                else pbExecutionStatus.Image = PMPSM310.Properties.Resources.ExecutionFail;
        ...
        private bool ExecuteProcedure()
        {
            try
            {
                ...
                this.Cursor = Cursors.WaitCursor;
                pbExecutionStatus.Image = PMPSM310.Properties.Resources.ExecutionWait;
                ...
                txtMeterPCBAID.Text = "";
                objappSettings.SetMeterMode((int)LayerInterface.MeterTypeInfo.Smart_Meter_3PH);
                ...
                while (selectedControlIDX < paraNameList.Count)
                {
                    ...
                    switch (ParaToBeExecute)
                    {
                        case "VERIFYPRODUCTSTAGE":
                            string tavelerreadResponse = objccmdmethod.GetTravelerData_3Phase(TravelerStage, (int)StaticVariables.ExecutedMeterType.Smart_Meter_3PH);
                        ...
                        case "VERIFYMETERPASSWORD":
                            ...
                            break;
                        case "CASETAMPERTEST":
                            string readcaseresponse = objccmdmethod.CheckingCase_SM310(IsFrontTested);
                            DGVParaLists.Rows[selectedControlIDX].Cells["colRemarks"].Value = readcaseresponse;
                            if (readcaseresponse.IndexOf(StaticVariables.ERRORPreFix) < 0) testExecutionStatus = (int)StaticVariables.ExecutionReurnStatus.Pass;
                            else testExecutionStatus = (int)StaticVariables.ExecutionReurnStatus.Fail;
                            break;
                        case "WRITEPCBAID":
                        ...
                        case "VERIFYCONFIGURATION":
                            string readconfigResponse = objccmdmethod.VerifyDisplayConfigData_3Phase(DGVParaLists.Rows[selectedControlIDX].Cells["colDefaultValue"].Value.ToString());
                        ...
                        case "VERIFYCALIBRATIONDATA":
                            string caliRespResponse = objccmdmethod.VerifyCalibrationData();
                        ...
                        case "RELAYCIRCUITTEST":
                            //MessageBox.Show("Switch OFF only Current ! Then Hit OK To Continue !", "L+G PMP", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                            //Application.DoEvents();
                            string relayCircuitsTestResponse = objccmdmethod.RelayCircuitTest_3Phase(DGVParaLists.Rows[selectedControlIDX].Cells["colDefaultValue"].Value.ToString(), DGVParaLists.Rows[selectedControlIDX].Cells["ColMinVal"].Value.ToString(), DGVParaLists.Rows[selectedControlIDX].Cells["ColMaxValue"].Value.ToString());
                            DGVParaLists.Rows[selectedControlIDX].Cells["colRemarks"].Value = relayCircuitsTestResponse;
                            if (relayCircuitsTestResponse.IndexOf(StaticVariables.ERRORPreFix) >= 0) { testExecutionStatus = (int)StaticVariables.ExecutionReurnStatus.Fail;/* DGVParaLists.Rows[selectedControlIDX].Cells["colStatus"].Value = "Fail"; DGVParaLists.Rows[selectedControlIDX].Cells["colStatus"].Style.BackColor = Color.Red; return false; */}
                            else testExecutionStatus = (int)StaticVariables.ExecutionReurnStatus.Pass;                           
                            break;
                        ...
                        case "SETMETERRTC":
                            ...
                            break;
                        case "PCBASTATUSTEST":
                            MessageBox.Show("Press UP, Down and MD Reset Switch ! Then Hit OK To Continue !", "L+G PMP", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                            Application.DoEvents();
                            string pcbastatusResponse = objccmdmethod.CheckingPCBASTATUS_3Phase((int)StaticVariables.ExecutedMeterType.Smart_Meter_3PH);
                            DGVParaLists.Rows[selectedControlIDX].Cells["colRemarks"].Value = pcbastatusResponse; ;
                        ...
                        case "MAGNETSENSORTESTLEFT":
                            MessageBox.Show("Apply Magnet To Left Sensor ! Then Hit OK To Continue !", "L+G PMP", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                            Application.DoEvents();
                            ...
                            break;
                        case "DIDOCIRCUITTEST":
                          
                            string statusdata = "",didoResponse="";
                            int readCounter=0;
                            while(readCounter < 5) //-----To Read all five flag's
                            {
                                if (readCounter > 0) MessageBox.Show("Switch ON DIDO Test Switch - " + readCounter, "L+G PMP", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                                 Application.DoEvents();
                                 didoResponse = objccmdmethod.DIDOCircuitTest(readCounter);
                                 statusdata += didoResponse + ",";
                                 if (didoResponse.IndexOf(StaticVariables.ERRORPreFix) >= 0) break;
                                 readCounter++;
                                
                            }
                            DGVParaLists.Rows[selectedControlIDX].Cells["colRemarks"].Value = statusdata;
                            if (statusdata.IndexOf(StaticVariables.ERRORPreFix) < 0) testExecutionStatus = (int)StaticVariables.ExecutionReurnStatus.Pass;
                            else testExecutionStatus = (int)StaticVariables.ExecutionReurnStatus.Fail;
                            break;
                        case "MAGNETSENSORTESTRIGHT":
                            MessageBox.Show("Apply Magnet To Right Sensor ! Then Hit OK To Continue !", "L+G PMP", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                            Application.DoEvents();
                        ...
                        case "NEUTRALCURRENTTEST":
                            //MessageBox.Show("Apply Neutral Current! Then Hit OK To Continue !", "L+G PMP", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                            Application.DoEvents();
                            string getneuCurrenetResponse = objccmdmethod.ReadMeterParameters_3Phase(DLMSDataStracture.CalibrationDataStracture.CalibrationNeutralCurrentReadOBIS, DLMSDataStracture.CalibrationDataStracture.CalibrationNonPowerReadClassID, (int)MeterParaRead.NeutralCurrent, DGVParaLists.Rows[selectedControlIDX].Cells["colDefaultValue"].Value.ToString(), DGVParaLists.Rows[selectedControlIDX].Cells["ColMinVal"].Value.ToString(), DGVParaLists.Rows[selectedControlIDX].Cells["ColMaxValue"].Value.ToString());
                        ...
                        case "CTDIRECTIONTEST":
                           // string getctDirectionResponse = objccmdmethod.ReadMeterParameters_3Phase(DLMSDataStracture.CTDirectionDataStracture.CTDirectionOBIS, DLMSDataStracture.CTDirectionDataStracture.CTDirectionClassID, (int)MeterParaRead.CTDirection, DGVParaLists.Rows[selectedControlIDX].Cells["colDefaultValue"].Value.ToString(), DGVParaLists.Rows[selectedControlIDX].Cells["ColMinVal"].Value.ToString(), DGVParaLists.Rows[selectedControlIDX].Cells["ColMaxValue"].Value.ToString());
                            string getctDirectionResponse = objccmdmethod.ReadCTDirectionFalconSM310();
                        ...
                        case "COMMUNICATIONTESTONBATTERYWOMAINS":
                            Application.DoEvents();
                            string getVoltageRPHonBattery = objccmdmethod.ReadMeterParameters_3Phase(DLMSDataStracture.EngineeringCommandDataStracture_3Phase.VoltageOBIS_RPhase, DLMSDataStracture.EngineeringCommandDataStracture_3Phase.EngineeringCommand_3Phase_ClassID, (int)MeterParaRead.Voltage_RPhase, DGVParaLists.Rows[selectedControlIDX].Cells["colDefaultValue"].Value.ToString(), DGVParaLists.Rows[selectedControlIDX].Cells["ColMinVal"].Value.ToString(), DGVParaLists.Rows[selectedControlIDX].Cells["ColMaxValue"].Value.ToString());
                            string getVoltageYPHonBattery = objccmdmethod.ReadMeterParameters_3Phase(DLMSDataStracture.EngineeringCommandDataStracture_3Phase.VoltageOBIS_YPhase, DLMSDataStracture.EngineeringCommandDataStracture_3Phase.EngineeringCommand_3Phase_ClassID, (int)MeterParaRead.Voltage_RPhase, DGVParaLists.Rows[selectedControlIDX].Cells["colDefaultValue"].Value.ToString(), DGVParaLists.Rows[selectedControlIDX].Cells["ColMinVal"].Value.ToString(), DGVParaLists.Rows[selectedControlIDX].Cells["ColMaxValue"].Value.ToString());
                            string getVoltageBPHonBattery = objccmdmethod.ReadMeterParameters_3Phase(DLMSDataStracture.EngineeringCommandDataStracture_3Phase.VoltageOBIS_BPhase, DLMSDataStracture.EngineeringCommandDataStracture_3Phase.EngineeringCommand_3Phase_ClassID, (int)MeterParaRead.Voltage_RPhase, DGVParaLists.Rows[selectedControlIDX].Cells["colDefaultValue"].Value.ToString(), DGVParaLists.Rows[selectedControlIDX].Cells["ColMinVal"].Value.ToString(), DGVParaLists.Rows[selectedControlIDX].Cells["ColMaxValue"].Value.ToString());
                            DGVParaLists.Rows[selectedControlIDX].Cells["colRemarks"].Value = "Voltage >>> R Phase =" + getVoltageRPHonBattery + ",  " + " Y Phase =" + getVoltageYPHonBattery + ",  " + " B Phase =" + getVoltageBPHonBattery;
                            if (getVoltageRPHonBattery.IndexOf(StaticVariables.ERRORPreFix) < 0 && getVoltageYPHonBattery.IndexOf(StaticVariables.ERRORPreFix) < 0 && getVoltageBPHonBattery.IndexOf(StaticVariables.ERRORPreFix) < 0) testExecutionStatus = (int)StaticVariables.ExecutionReurnStatus.Pass;
                            else testExecutionStatus = (int)StaticVariables.ExecutionReurnStatus.Fail;
                            break;   
                       ...
                       case "RESCANMETERID":
                        ...
                        case "SETDEFAULTPASSWORD":
                            ...
                            break;
                        case "JTAGLOCKING":
                            string jTagLockingResponse = objccmdmethod.JTagLocking(true);
                            DGVParaLists.Rows[selectedControlIDX].Cells["colRemarks"].Value = jTagLockingResponse;
                            if (jTagLockingResponse.IndexOf(StaticVariables.ERRORPreFix) < 0) testExecutionStatus = (int)StaticVariables.ExecutionReurnStatus.Pass;
                            else testExecutionStatus = (int)StaticVariables.ExecutionReurnStatus.Fail;
                            break; 
                        case "JTAGUNLOCKING":
                            string jTagUnlockingResponse = objccmdmethod.JTagLocking(false);
                            DGVParaLists.Rows[selectedControlIDX].Cells["colRemarks"].Value = jTagUnlockingResponse;
                            if (jTagUnlockingResponse.IndexOf(StaticVariables.ERRORPreFix) < 0) testExecutionStatus = (int)StaticVariables.ExecutionReurnStatus.Pass;
                            else testExecutionStatus = (int)StaticVariables.ExecutionReurnStatus.Fail;
                            break;
                        case "LOCKFSMODE":
                ...
                if (objexeresult.ExecutionProcedureType.IndexOf(StaticVariables.TestType_OT) < 0)
                {
                    ...
                    {
                        //Thread.Sleep(1000);
                        if (!objccmdmethod.SetTravelerData_3Phase(selectedControlIDX, TravelerStage, DGVParaLists)) { MessageBox.Show("Unable To Set Production Stage !", "L+G PMP", MessageBoxButtons.OK, MessageBoxIcon.Error); return false; }
                    }
